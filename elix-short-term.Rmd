...
for(i in vars_to_obfs) {
    ...
    table_mat <- table_mat %>%
      mutate(`% No neurological condtion` = round(`No neurological condition`/sum_no_neuro*100, 1),
             `% Has neurological condition` = round(`Has neurological condition`/sum_neuro*100, 1))
    
    colnames(table_mat)[1] <- paste(i)
    
    table_mat <- table_mat %>%
      select(paste(i), `No neurological condition`, `% No neurological condtion`, 
             `Has neurological condition`, `% Has neurological condition`, Total) %>%
