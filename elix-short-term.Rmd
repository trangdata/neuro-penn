...
## obfuscation
vars_to_obfs <- c("Sex", "Age group", "Race", "Severity",
                  "Survival", "Readmitted")

obfs_list <- list()
for(i in vars_to_obfs) {
  
    table_mat <-  as.data.frame(table(formatted_demo$neuro_post,
                                      formatted_demo[[i]]))
    
    nrow_var <- nrow(table_mat)
    ncol_var <- ncol(table_mat)
    
    blur_vec <- matrix(sample(seq(-blur_abs, blur_abs),
                                nrow_var*length(table_mat),
                                replace = TRUE), ncol = ncol_var)
    
    # choose random variable to indicate which column of our blur_vec should be used to blur the data
    ran_col_to_blur <- sample(1:ncol_var, 1)
    
    table_mat <- table_mat %>%
      mutate(Freq = ifelse(Freq < mask_thres, 0, Freq + blur_vec[,ran_col_to_blur])) %>%
      pivot_wider(names_from = "Var1", 
                values_from = "Freq") %>%
      rowwise() %>%
      mutate(Total = sum(`Has neurological condition`, `No neurological condition`))
    
    sum_no_neuro = sum(table_mat$`No neurological condition`)
    sum_neuro = sum(table_mat$`Has neurological condition`)
    sum_all <- sum_no_neuro + sum_neuro
    
    table_mat <- table_mat %>%
      mutate(`% No neurological condition` = round(`No neurological condition`/sum_no_neuro*100, 1),
             `% Has neurological condition` = round(`Has neurological condition`/sum_neuro*100, 1),
             `% Total` = round(Total/sum_all*100, 1))
    
    colnames(table_mat)[1] <- paste(i)
    
    table_mat <- table_mat %>%
      select(paste(i), `No neurological condition`, `% No neurological condition`, 
             `Has neurological condition`, `% Has neurological condition`, Total, `% Total`) %>%
      remove_rownames %>% column_to_rownames(var=paste(i))

    obfs_list[[i]] <- table_mat
}

demo_table <- do.call(rbind.data.frame, obfs_list)

## Add time to severity and time to death
neuro_cond <- t(as.data.frame(table(formatted_demo$neuro_post)))
neuro_cond <- as.data.frame.matrix(neuro_cond)  %>% mutate_all(as.character)
row.names(neuro_cond) <- NULL
colnames(neuro_cond) <- neuro_cond[1,]
neuro_cond <- neuro_cond[-1,]
row.names(neuro_cond) <- "Total"
  
# severity                
severity_demo_table <- severity_stats(formatted_demo, neuro_cond)

# death 
death_demo_table <- death_stats(formatted_demo, neuro_cond)

# print demographics table
datatable(demo_table)
# time to severity
datatable(severity_demo_table)
# time to 
datatable(death_demo_table)
...
#)
  ...
  ## print table 1 html - no obfuscation
table1::table1(
...
list_results = list(
  ...
  severity_demo_table = severity_demo_table,
  death_demo_table = death_demo_table,
