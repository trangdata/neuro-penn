...
comorb_elix <- map_charlson_codes(df = obs_raw,
...
comorb_unique <- 
  # full_join(
  # index_scores %>% select(patient_num, charlson_score), 
...
comorb_unique %>% 
  ...
  ggplot(aes(x = neuro_post, y = van_walraven_score, fill = neuro_post)) + 
...
scores_neuro <- obs_raw %>% 
  ...
  mutate(concept_code = case_when(
           ...
           TRUE ~ concept_code) %>% 
           as.factor() %>% 
           fct_reorder(- van_walraven_score),
...
comorb_long <- scores_unique %>%
  select(patient_num, neuro_post, van_walraven_score, any_of(pos_comorbs)) %>%
  pivot_longer(any_of(comorbs_elix)) %>%
  filter(value == 1)
...
comorb_wilcox_func <- function(comorb, df = comorb_long) {
  ...
  try(wilcox.test(neuro$van_walraven_score, non_neuro$van_walraven_score)$p.value, silent = TRUE)
}
...
```{r}
scores_unique_elix <- scores_unique %>% 
  # filter(score == "van_walraven_score") %>%
  # rename("ElixhauserScore" = value) %>% 
  {.}

severe_reg_elix <- glm(severe ~ neuro_post + van_walraven_score + sex + age_group + race,
   data = scores_unique_elix, family = 'binomial') %>% 
  tidy()
severe_reg_elix
deceased_reg_elix <- glm(deceased ~ neuro_post + van_walraven_score + sex + age_group + race,
   data = scores_unique_elix, family = 'binomial') %>% 
  tidy()
deceased_reg_elix
```
...
```{r}
n_stay_reg_elix <- lm(n_stay ~ neuro_post + van_walraven_score + race + sex + age_group, 
...
multi_comorb <- function(x, ...) {
  process_tables(
    map_charlson_codes(obs_raw, comorb_names_elix, x$t1, x$t2, 
...
obs_first <- obs_raw %>% 
  group_by(patient_num) %>% 
  merge(mapped_codes_table_elix, by = "concept_code") %>%
...
n_stay_reg_penn = n_stay_reg_elix
severe_reg_penn = severe_reg_elix
deceased_reg_penn = deceased_reg_elix
...
write.csv(combined_table1_elix, "results/penn_combined_elix.csv", row.names = FALSE)
```
