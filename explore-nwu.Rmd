...
demo_raw <- read_csv('../../Phase2.1/Date_Shifted_Files/LocalPatientSummary_Shift.csv')
obs_raw <- read_csv('../../Phase2.1/LocalPatientObservations.csv')
labs_raw <- obs_raw %>% filter(concept_type == "LAB_LOINC")

# modify dates of '1900-01-01'
demo_raw$last_discharge_date[demo_raw$last_discharge_date == "1900-01-01"] <- NA
demo_raw$severe_date[demo_raw$severe_date == "1900-01-01"] <- NA
demo_raw$death_date[demo_raw$death_date == "1900-01-01"] <- NA
```
...
readmissions <- clin_raw %>% 
  ...
  mutate(n_readmissions = (status_changes - 1)) %>% 
...
demo_df <- demo_raw %>% 
  # please comment out the three following lines of renamew,
