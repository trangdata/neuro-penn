...
# considers only codes between -14 & -1 days prior to admission as comorbidities
obs_raw14_1 <- obs_raw %>% filter(days_since_admission >= -14 & days_since_admission <= -1)
comorb14_1 <- map_charlson_codes(obs_raw14_1, comorb_names, -1) 

# map only day of admission
obs_raw0 <- obs_raw %>% filter(days_since_admission == 0)
comorb_onlydayOf <- map_charlson_codes(obs_raw0, comorb_names, 0) 

# consider codes only codes after admission
obs_raw1 <- obs_raw %>% filter(days_since_admission >= 1)
comorb_After <- map_charlson_codes(obs_raw1, comorb_names, 1) 

# -15 to -365 days prior to admission
index_scores15 <- comorb15$index_scores
mapped_codes15 <- comorb15$mapped_codes_table

day15_table1 <- get_table1(
  index_scores15 %>% filter(patient_num %in% neuro_pt_post)) %>% 
  rename('n_neuro_pats' = n_patients) %>% 
  left_join(get_table1(index_scores15), .,
            by = c("Comorbidity", "Abbreviation")) %>%
  mutate(time_map = "15 to 365 days prior admisison")


# day of and prior to admission
index_scores_dayOf <- comorb0$index_scores
mapped_codes_dayOf <- comorb0$mapped_codes_table
...
dayOf_table1 <- get_table1(
  ...
  left_join(get_table1(index_scores_dayOf), .,
            by = c("Comorbidity", "Abbreviation"))%>%
  mutate(time_map = "0 to -365 days admission")

# -14 to -1 days before admission
index_scores14_1 <- comorb14_1$index_scores
mapped_codes14_1 <- comorb14_1$mapped_codes_table

day14_table1 <- get_table1(
  index_scores14_1 %>% filter(patient_num %in% neuro_pt_post)) %>% 
  rename('n_neuro_pats' = n_patients) %>% 
  left_join(get_table1(index_scores14_1), .,
            by = c("Comorbidity", "Abbreviation")) %>%
  mutate(time_map = "14 to 1 days prior admisison")


# Only Day of Admission
index_scores_onlydayOf <- comorb_onlydayOf$index_scores
mapped_codes_onlydayOf <- comorb_onlydayOf$mapped_codes_table

onlydayOf_table1 <- get_table1(
  index_scores_onlydayOf %>% filter(patient_num %in% neuro_pt_post)) %>% 
  rename('n_neuro_pats' = n_patients) %>% 
  left_join(get_table1(index_scores_onlydayOf), .,
            by = c("Comorbidity", "Abbreviation")) %>%
  mutate(time_map = "day of admission")

combined_table1_mod <- combined_table1 %>% 
  mutate(time_map = "prior to admission") %>%
  select(-prop_patients, -prop_neuro_pats)

# day after admission
# -15 to -365 days prior to admission
index_scores_After <- comorb_After$index_scores
mapped_codes_After <- comorb_After$mapped_codes_table

After_table1 <- get_table1(
  index_scores_After %>% filter(patient_num %in% neuro_pt_post)) %>% 
  rename('n_neuro_pats' = n_patients) %>% 
  left_join(get_table1(index_scores_After), .,
            by = c("Comorbidity", "Abbreviation")) %>%
  mutate(time_map = "after admission")

all_tables <- rbind(combined_table1_mod, day15_table1, day14_table1, onlydayOf_table1, After_table1)
```

### Timeline - All Patients
```{r}
all_tables$time_map <- factor(all_tables$time_map,
                              levels = c("prior to admission",
                                         "15 to 365 days prior admisison",
                                         "14 to 1 days prior admisison",
                                         "day of admission",
                                         "after admission"))
...
### Timeline - Neuro patients
```{r}
ggplot(all_tables,
           aes(x = time_map,  y = n_neuro_pats, fill = time_map)) +
    geom_bar(stat = "identity") + 
   facet_wrap(~Abbreviation) + 
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
## Charlson Score Overtime 
```{r}
t1 <- median(index_scores$charlson_score)
t2 <- median(index_scores15$charlson_score)
t3 <- median(index_scores14_1$charlson_score)
t4 <- median(index_scores_dayOf$charlson_score)
t5 <- median(index_scores_After$charlson_score)

char_time <- as.data.frame(rbind(t1, t2, t3, t4, t5))
row.names(char_time) <- c("prior to admission", "-15-365", "-14-1", "day of admission", 
                          "after admission")
colnames(char_time) <- "Median_Charlson_Score"
char_time <- tibble::rownames_to_column(char_time, "Time")

char_time$Time <- factor(char_time$Time,
                              levels = c("prior to admission",
                                         "-15-365",
                                         "-14-1",
                                         "day of admission",
...
index_scores_neuro <- index_scores %>% filter(patient_num %in% neuro_pt_post)
index_scores_neuro15 <- index_scores15 %>% filter(patient_num %in% neuro_pt_post)
index_scores_neuro14_1 <- index_scores14_1 %>% filter(patient_num %in% neuro_pt_post)
index_scores_neuro_dayOf <- index_scores_dayOf %>% filter(patient_num %in% neuro_pt_post)
index_scores_neuro_After <- index_scores_After %>% filter(patient_num %in% neuro_pt_post)

t1n <- median(index_scores_neuro$charlson_score)
t2n <- median(index_scores_neuro15$charlson_score)
t3n <- median(index_scores_neuro14_1$charlson_score)
t4n <- median(index_scores_neuro_dayOf$charlson_score)
t5n <- median(index_scores_neuro_After$charlson_score) 

char_time_neuro <- as.data.frame(rbind(t1n, t2n, t3n, t4n, t5n))
row.names(char_time_neuro) <- c("prior to admission", "-15-365", "-14-1", "day of admission", 
                          "after admission")
colnames(char_time_neuro) <- "Median_Charlson_Score"
char_time_neuro <- tibble::rownames_to_column(char_time_neuro, "Time")

char_time_neuro$Time <- factor(char_time_neuro$Time,
                              levels = c("prior to admission",
                                         "-15-365",
                                         "-14-1",
                                         "day of admission",
...
char_time_neuro$cohort <- "Neuro"
char_time$cohort <- "All Patients"

char_time_all <- rbind(char_time_neuro, char_time)

ggplot(char_time_all, aes(x = Time, y = Median_Charlson_Score, fill = cohort)) + 
...
Perhaps could go a conservative route and only include comorbidities if there was a previous record of it? However, comorbidities can contain multiple different types of diseases. So perhaps could be problematic.
```{r}
obs_first <- obs_raw %>% 
  group_by(patient_num) %>% 
