...
neuro_pt_prior <- demo_processed %>% 
  filter(neuro_post) %>% 
  pull(patient_num) %>% 
  unique()

length(neuro_pt_prior)
```
...
combined_table1 <- get_table1(
  index_scores %>% filter(patient_num %in% neuro_pt_prior)) %>% 
...
dayOf_table1 <- get_table1(
  index_scores_dayOf %>% filter(patient_num %in% neuro_pt_prior)) %>% 
  rename('n_neuro_pats' = n_patients) %>% 
