...
```{r}
obs_raw <- obs_raw %>% filter(concept_type == "DIAG-ICD10")

neuro_pts <- obs_raw %>% 
  filter(days_since_admission >= 0) 
neuro_pts <- merge(neuro_pts, neuro_icds_10, by = 'concept_code')

nrow(unique(data.frame(neuro_pts$patient_num)))
```
...
neuro_obs_prior <- obs_raw %>%
  filter(days_since_admission < 0) %>%
  filter(patient_num %in% neuro_pts$patient_num)

nrow(unique(data.frame(neuro_obs_prior$patient_num)))  
```
...
```{r}
nrow(data.frame(comorb[[2]]$patient_num))

nrow(data.frame(comorb_neuro[[2]]$patient_num))
```
...
```{r}
full_table1 <- comorb[[4]]
neuro_comorbs_table1 <- comorb_neuro[[4]]

combined_table1 <- left_join(full_table1, neuro_comorbs_table1, 
                             by = "Comorbidity")
colnames(combined_table1) <- c("Comorbidity", "Full_Cohort", "Full_Cohort_perc", "Neuro_Cohort", "Neuro_Cohort_perc")
...
combined_table1 <- as.data.frame(combined_table1)
...
```{r}
# obtain comorb. mapping from the full cohort
icd_map <- comorb[[2]]
...
neuro_matrix <- t(dcast(obs_neuro_unique, concept_code ~ patient_num, value.var="patient_num", length))
neuro_matrix <- as.data.frame(neuro_matrix)
neuro_matrix <- tibble::rownames_to_column(neuro_matrix, "patient_num")
neuro_matrix <- data.frame(lapply(neuro_matrix, as.character), stringsAsFactors=FALSE)
names(neuro_matrix) <- neuro_matrix[1,]
neuro_matrix <- neuro_matrix[-1,]
colnames(neuro_matrix)[1] <- "patient_num"

# merge neuro information to our comorbidity map
neuro_comorbs <- left_join(icd_map, neuro_matrix, by = "patient_num")

# add severity, death, and demographic information
demo <- demo_raw %>% 
  select(patient_num, severe, age_group, deceased)

neuro_comorbs <- merge(neuro_comorbs, demo, by = "patient_num")
...
neuro_comorbs <- merge(neuro_comorbs, scores, by = "patient_num")
```

### Indicate the Neuro patients
```{r}
neuro_pt_ids <- neuro_pts %>% 
  distinct(patient_num) %>%
  mutate(neuro = 1)

neuro_comorbs <- left_join(neuro_comorbs, neuro_pt_ids, by = "patient_num")
neuro_comorbs$neuro[is.na(neuro_comorbs$neuro)] <- 0
```
...
```{r}
ggplot(neuro_comorbs) +
  geom_histogram(bins = 15, aes(x = charlson_score, fill = "gray")) +
  geom_histogram(bins = 15, data = subset(neuro_comorbs, neuro == "1"), aes(x = charlson_score, fill = "slateblue")) + 
